...
async def main():
    ...
    await move_pair_tank_forward(45, 0, 250)# turn so the attatchment can do the Forge mission

    # Go to forge
    await move_pair_forward(560, 0, 500)# move forward so the wheelcan hit the stick to let the balls out

    await move_motor_backward(150, 500)# backward arm to make sure it does not get in the way

    await move_pair_backward(100, 0, 700)# go backward to be able to do the


    await move_pair_tank_forward(260, 0, 600)# first turn to turn toward mission
