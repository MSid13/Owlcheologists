...
async def main():
    ...
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -5, 0, velocity=700
    )  # alignment by pusing backwards
    ...
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, -5, 0, 600
    )  # When the attactchment hits, turns bot- conteracts turn
    ...
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, 40, 0, 250
    )  # turn so the attatchment can do the Forge mission

    # Go to forge
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, 650, 0, velocity=500
    )  # move forward so the wheelcan hit the stick to let the balls out

    await motor.run_for_degrees(
        right_motor_port, -350, 500
    )  # backward arm to make sure it does not get in the way

    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -180, 0, velocity=700
    )  # go backward to be able to do the
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, 270, 0, 600
    )  # first turn to turn toward mission
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, 400, 0, velocity=400
    )  # go forward to the mission
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, 70, 0, 600
    )  # second turn to turn toward mission
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -200, 0, velocity=500
    )  # push back to complete mission
    await move_straight_for_degrees(
        port.F, port.E, 230, 550
    )  # go forward to be able to turn to do whats on sale
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, 500, 0, 500
    )  # turn to whats on sale

    await move_straight_for_degrees(port.F, port.E, 500, 400)  # Push wahts on sale

    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, 10, 0, 600
    )  ## TURN to offset any turn happening

    await move_straight_for_degrees(port.F, port.E, 450, 800)  # Push at higher speed

    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -180, 0, velocity=500
    )  # Go back to get out of whats on sale
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, -160, 0, 600
    )  # Turn toward to tip the scales
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, 350, 0, velocity=500
    )  # Go forward towards tip the scales

    await motor.run_for_degrees(right_motor_port, 160, 10000)  # tip the scales
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -160, 0, velocity=400
    )  # go back out of tip the scales
    await motor_pair.move_tank_for_degrees(
        motor_pair.PAIR_1, -505, 0, 600
    )  # turn towards base
    await motor_pair.move_for_degrees(
        motor_pair.PAIR_1, -1600, 0, velocity=800
    )  # go to base
